...
const MAX_PUBLIC_TRANSLATION_CHARS = 450;
const FALLBACK_TIMEOUT_MS = 3500;
const CONFIGURED_PROVIDER_TIMEOUT_MS = 10_000;
...
export async function translateLongText(text, sourceLanguage = 'auto', targetLanguage = 'en') {
  ...
  const chunks = chunkText(value, MAX_PUBLIC_TRANSLATION_CHARS);
...
export async function translateLongTextWithConfiguredProvider(text, sourceLanguage = 'auto', targetLanguage = 'en') {
  const value = String(text || '').trim();
  if (!value || !process.env.TRANSLATION_API_URL) return value;
  const resolvedSourceLanguage = sourceLanguage === 'auto' ? detectLikelySourceLanguage(value) : sourceLanguage;
  const chunks = chunkText(value, MAX_PUBLIC_TRANSLATION_CHARS);
  const translations = await mapWithConcurrency(chunks, TRANSLATION_CONCURRENCY, async (chunk) => {
    const translated = await translateWithConfiguredProvider(chunk, resolvedSourceLanguage, targetLanguage);
    return isUsableTranslation(translated, chunk) ? translated : chunk;
  });
  return translations.join(' ').replace(/\s+/g, ' ').trim() || value;
}
...
async function translateTextUncached(text, sourceLanguage, targetLanguage) {
  ...
  if (targetLanguage === 'en' || isFallbackTranslationSupported(targetLanguage)) {
    ...
    return isUsableTranslation(translated, text) ? translated : text;
  }
  return text;
}
...
async function translateOne(apiUrl, apiKey, text, sourceLanguage, targetLanguage) {
  try {
    const response = await fetch(apiUrl, {
      ...
      signal: AbortSignal.timeout(CONFIGURED_PROVIDER_TIMEOUT_MS),
    });
...
async function translateWithPublicFallback(text, sourceLanguage, targetLanguage) {
  const safeText = String(text || '').slice(0, MAX_PUBLIC_TRANSLATION_CHARS);
  if (sourceLanguage !== 'auto') {
    const myMemoryTranslation = await translateWithMyMemoryFallback(safeText, sourceLanguage, targetLanguage);
    if (isUsableTranslation(myMemoryTranslation, safeText)) return myMemoryTranslation;
  }
  const googleTranslation = await translateWithGoogleFallback(safeText, sourceLanguage, targetLanguage);
  return isUsableTranslation(googleTranslation, safeText) ? googleTranslation : text;
}
...
function isUsableTranslation(translated, original) {
  ...
  if (!value || value === String(original || '').trim()) return false;
  if (/QUERY LENGTH LIMIT EXCEEDED/i.test(value)) return false;
  if (/MAX ALLOWED QUERY/i.test(value)) return false;
  if (/MYMEMORY WARNING/i.test(value)) return false;
  return true;
}
