...
const systemPhrasePatterns = [
  /\b(?:automated\s+)?[a-z][a-z-]*(?:\s+[a-z][a-z-]*){0,4}\s+(?:system|tool|engine|model|algorithm)\b/gi,
...
function normalizeEntities(value, sourceText = '') {
  ...
  normalized.people_roles = compactEntityList(normalized.people_roles.map(cleanEntity));
  return normalized;
}
...
function extractSystemPhrases(text) {
  ...
  for (const pattern of systemPhrasePatterns) {
    for (const match of text.matchAll(pattern)) {
      matches.push(cleanEntity(match[0]));
    }
  }
  return matches;
}
