...
const MAX_TASK_TIMEOUT_MS = 20000;
...
async function tryRunTask25Locally(text) {
  if (!text || process.env.TASK25_DISABLE_LOCAL_RUNNER === 'true') return null;
  ...
  try {
    ...
    if (!row) return null;
    return task25LocalPayload(row, { text });
  } catch (error) {
...
function task25LocalPayload(row, { text = '', impactFallback = null } = {}) {
  ...
  const impact = chooseImpactResult(row, impactFallback);
  return {
    inputField: 'narrativeText',
    ...
    task2: {
      ...
      modelSource: impact.modelSource,
    },
    task3: {
      status: 'COMPLETED',
      tool: TASK3_MODEL,
      model: TASK3_MODEL,
      runtime: 'local-python',
      aiThemes: Array.isArray(row.aiThemes) ? row.aiThemes : [],
    },
    task4: {
      status: 'COMPLETED',
      tool: TASK4_TOOL,
...
function chooseImpactResult(row, fallback) {
  const modelResult = {
    aiImpactClassification: row.aiImpactClassification || 'UNCLEAR',
    aiConfidenceScore: roundScore(row.aiConfidenceScore || 0),
    humanReviewRequired: Boolean(row.humanReviewRequired),
    evidenceScores: row.evidenceScores || {},
    modelSource: 'local-python-model',
  };
  if (!fallback) return modelResult;
  const fallbackConfidence = Number(fallback.aiConfidenceScore || 0);
  if (modelResult.aiConfidenceScore >= 0.5 && modelResult.aiImpactClassification === fallback.aiImpactClassification) {
    return modelResult;
  }
  if (modelResult.aiConfidenceScore < 0.5 || fallbackConfidence >= 0.85) {
    return {
      aiImpactClassification: fallback.aiImpactClassification,
      aiConfidenceScore: roundScore(Math.max(fallbackConfidence, modelResult.aiConfidenceScore)),
      humanReviewRequired: fallback.humanReviewRequired,
      evidenceScores: fallback.evidenceScores || modelResult.evidenceScores,
      modelSource: 'local-python-model + local evidence calibration',
    };
  }
  return modelResult;
}
...
async function classifyImpact(text) {
  ...
  try {
    const output = await zeroShot(TASK2_MODEL, process.env.ML_IMPACT_ENDPOINT || process.env.ML_BART_ENDPOINT, text, Object.values(impactLabels), 'This public service story has this impact: {}', false);
    ...
    const classification = Object.entries(impactLabels).find(([, label]) => label === output.labels[0])?.[0] || 'UNCLEAR';
    const confidence = Number(output.scores[0] || 0);
    return {
...
async function detectThemes(text) {
  ...
  return mergeThemeEntries(output ? entries : evidenceFallbackEntries).slice(0, 8);
}
