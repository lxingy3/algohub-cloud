...
async function classifyImpact(text) {
  const output = await zeroShot(TASK2_MODEL, process.env.ML_DEBERTA_ENDPOINT, text, Object.values(impactLabels), 'This public service story has this impact: {}');
...
async function detectThemes(text) {
  ...
  const output = await zeroShot(TASK3_MODEL, process.env.ML_BART_ENDPOINT, text, descriptions, 'This public service story shows this theme: {}');
...
async function extractKeywords(text) {
  ...
  return normalizeKeywords(output.keywords || output);
}

async function zeroShot(model, endpoint, text, candidateLabels, hypothesisTemplate) {
  const payload = {
    text,
    inputs: text,
...
function normalizeEntities(value) {
  ...
  return Object.fromEntries(groups.map((group) => [group, uniqueValues(Array.isArray(value[group]) ? value[group] : [])]));
}
