import { assessMlNarrativeScope } from './mlDomain.js';
...
export async function analyzeNarrativeTextWithModels(narrativeText) {
  ...
  const scope = assessMlNarrativeScope(text);
  if (!scope.inScope) {
    return skippedOutOfScopePayload(scope.reason);
  }
...
function skippedOutOfScopePayload(reason) {
  return {
    inputField: 'narrativeText',
    source: 'model',
    status: 'OUT_OF_SCOPE',
