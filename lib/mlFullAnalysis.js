...
const TASK6_TOOL = 'local algorithm registry linker';
const TASK7_TOOL = 'local summary rules';
...
const TASK6_MIN_CONFIDENCE = 0.28;
const TASK6_MAX_MATCHES = 5;

const task6StopWords = new Set([
  'the',
  'and',
  'for',
  'with',
...
export async function analyzeNarrativeTextWithModels(narrativeText, options = {}) {
  ...
  const task4Payload = taskPayload(task4Result, 'entity extraction', (value) => ({ entities: value }), TASK4_TOOL);
  const task5Payload = taskPayload(task5Result, 'keyword extraction', (value) => ({ keywords: value }), TASK5_TOOL);
  const task6Result = await settleTask(() => linkAlgorithmsForTask6({
    text,
    entities: task4Payload.status === 'COMPLETED' ? task4Payload.entities : {},
    keywords: task5Payload.status === 'COMPLETED' ? task5Payload.keywords : [],
    affectedDomain: options.affectedDomain,
    algorithms: options.algorithms,
  }));
  const task7Result = await settleTask(() => summarizeForTask7(text));
  const taskPayloads = [
    task2Result,
    task3Result,
    task4Result,
  ...
  return {
    ...
    source: 'model',
    status: taskPayloads.every((result) => result.status === 'fulfilled') ? 'COMPLETED' : 'PARTIAL',
    task1: {
      status: 'SKIPPED',
    ...
    task4: task4Payload,
    task5: task5Payload,
    task6: taskPayload(task6Result, 'algorithm linking', (value) => ({ linkedAlgorithms: value }), TASK6_TOOL),
    task7: taskPayload(task7Result, 'summarization', (value) => ({ summary: value }), TASK7_TOOL),
  };
}

export async function analyzeNarrativeTextTask4To7(narrativeText, options = {}) {
  const text = String(narrativeText || '').trim();
  ...
  return {
    ...
    task2: {
      status: 'SKIPPED',
      reason: 'Task 4-7 refresh preserves existing impact classification.',
    },
    task3: {
      status: 'SKIPPED',
      reason: 'Task 4-7 refresh preserves existing theme detection.',
    },
    task4: task4Payload,
    task5: task5Payload,
    task6: taskPayload(task6Result, 'algorithm linking', (value) => ({ linkedAlgorithms: value }), TASK6_TOOL),
...
export async function linkAlgorithmsForTask6({ text, entities = {}, keywords = [], affectedDomain = '', algorithms = [] }) {
  if (!Array.isArray(algorithms) || algorithms.length === 0) {
    throw new Error('algorithm linking endpoint is not configured with registry candidates.');
  }

  const normalizedText = normalizeTask6Text([
    text,
    affectedDomain,
    ...Object.values(entities || {}).flat(),
    ...keywords,
  ].join(' '));
  const testimonyTokens = tokenSet(normalizedText);
  const keywordPhrases = normalizePhraseList(keywords);
  const agencyPhrases = normalizePhraseList(entities.agencies);
  const systemPhrases = normalizePhraseList(entities.systems);
  const domainText = normalizeTask6Text(affectedDomain);

  return algorithms
    .map((algorithm) => scoreAlgorithmCandidate({
      algorithm,
      normalizedText,
      testimonyTokens,
      keywordPhrases,
    ...
    .filter((candidate) => candidate.confidence >= TASK6_MIN_CONFIDENCE)
    .sort((a, b) => b.confidence - a.confidence || a.name.localeCompare(b.name))
    .slice(0, TASK6_MAX_MATCHES);
}

export function summarizeForTask7(text) {
  const cleaned = cleanSummaryText(text);
  if (!cleaned) return '';
  const sentences = splitSummarySentences(cleaned);
  const selected = selectTask7SummarySentences(sentences, cleaned);
  const base = selected.length ? selected.join(' ') : cleaned.slice(0, 320);
  return trimSummaryToWord(base, 320);
}
...
function selectTask7SummarySentences(sentences, cleanedText) {
  if (!sentences.length) return [];
  if (sentences.length <= 2 && cleanedText.length <= 320) return sentences;

  const ranked = sentences
    .map((sentence, index) => ({
      sentence,
      index,
      score: scoreTask7SummarySentence(sentence, index, sentences.length),
    }))
  ...
  for (const item of ranked) {
    ...
    const next = [...selected, item].sort((a, b) => a.index - b.index);
    const nextText = next.map((entry) => entry.sentence).join(' ');
    if (nextText.length <= 320) selected.push(item);
  }

  if (!selected.length) {
    return sentences.slice(0, 2).filter(Boolean);
  }

  if (selected.length === 1) {
    const context = findBestSummaryContextSentence(sentences, selected[0], selected[0].score);
    if (context && `${selected[0].sentence} ${context.sentence}`.length <= 320) {
...
function scoreTask7SummarySentence(sentence, index, totalSentences) {
  const normalized = normalizeTask6Text(sentence);
  const tokens = tokenSet(normalized);
  if (!normalized || tokens.size < 3) return -5;

  let score = 0;
  if (index === 0) score += 0.25;
  if (index === totalSentences - 1) score += 0.15;
  if (sentence.length >= 70 && sentence.length <= 240) score += 0.6;
  if (sentence.length > 280) score -= 1;

  score += summaryPatternScore(normalized, [
    'algorithm',
    'automated',
    'computer',
    'decision',
  ...
  if (/\b(?:i|my|we|our|family|resident|applicant|parent|worker|tenant|student)\b/.test(normalized)) score += 0.8;
  if (/\b(?:felt|feel|trust|listen|support|punish|pressure|harm|helped)\b/.test(normalized)) score += 0.9;
  if (/^(?:hello|hi|thanks|thank you|i remember when|this is a test)\b/.test(normalized)) score -= 2.5;
  if (/\b(?:community meeting|public technology can be hard|specific system story yet)\b/.test(normalized)) score -= 1.5;

  return score;
}

function summaryPatternScore(text, phrases, weight) {
  const matches = phrases.reduce((count, phrase) => count + (text.includes(phrase) ? 1 : 0), 0);
  return Math.min(weight, matches * (weight / 2));
}

function findBestSummaryContextSentence(sentences, selected, selectedScore) {
  const candidates = [selected.index - 1, selected.index + 1]
    .filter((index) => index >= 0 && index < sentences.length)
    .map((index) => ({
      sentence: sentences[index],
      index,
      score: scoreTask7SummarySentence(sentences[index], index, sentences.length),
    }))
    .filter((item) => item.score >= Math.max(0.5, selectedScore - 3));
  return candidates.sort((a, b) => b.score - a.score || a.index - b.index)[0] || null;
}

function scoreAlgorithmCandidate({
  algorithm,
  normalizedText,
  testimonyTokens,
  keywordPhrases,
  ...
  const algorithmText = normalizeTask6Text([
    algorithm.name,
    algorithm.useCase,
  ...
  const algorithmTokens = tokenSet(algorithmText);
  const algorithmName = normalizeTask6Text(algorithm.name);
  const algorithmUseCase = normalizeTask6Text(algorithm.useCase);
  const algorithmAgency = normalizeTask6Text(algorithm.agencyName);

  const nameMatch = phraseMatchScore([algorithmName], normalizedText);
  const useCaseMatch = phraseMatchScore([algorithmUseCase], normalizedText);
  const keywordOverlap = phraseOverlapScore(keywordPhrases, algorithmText, algorithmTokens);
  const agencyMatch = phraseMatchScore(agencyPhrases, algorithmAgency || algorithmText);
  const systemMatch = Math.max(
    phraseMatchScore(systemPhrases, algorithmText),
    phraseMatchScore([algorithmName], systemPhrases.join(' ')),
  );
  const entityMatch = Math.max(agencyMatch, systemMatch);
  const domainMatch = domainText
    ? Math.max(
        phraseMatchScore([domainText], algorithmUseCase || algorithmText),
        tokenOverlapScore(tokenSet(domainText), algorithmTokens),
      )
    : 0;
  const textOverlap = tokenOverlapScore(testimonyTokens, algorithmTokens);
  ...
  return {
    ...
    method: 'local_keyword_entity_registry',
    scoreBreakdown: {
      nameMatch: roundScore(nameMatch),
      keywordOverlap: roundScore(keywordOverlap),
      entityMatch: roundScore(entityMatch),
      domainMatch: roundScore(domainMatch),
      useCaseMatch: roundScore(useCaseMatch),
      textOverlap: roundScore(textOverlap),
    },
  };
}

function phraseMatchScore(phrases, haystack) {
  const target = normalizeTask6Text(haystack);
  if (!target) return 0;
  return normalizePhraseList(phrases).reduce((best, phrase) => {
    if (!phrase) return best;
    if (target.includes(phrase) || phrase.includes(target)) return Math.max(best, 1);
    return Math.max(best, tokenOverlapScore(tokenSet(phrase), tokenSet(target)));
  }, 0);
}

function phraseOverlapScore(phrases, algorithmText, algorithmTokens) {
  const normalizedPhrases = normalizePhraseList(phrases);
  if (!normalizedPhrases.length) return 0;
  let score = 0;
  for (const phrase of normalizedPhrases) {
    if (algorithmText.includes(phrase)) {
      score += 1;
    } else {
      score += tokenOverlapScore(tokenSet(phrase), algorithmTokens);
    }
  }
  return clampScore(score / normalizedPhrases.length);
}
...
function tokenOverlapScore(leftTokens, rightTokens) {
  ...
  return clampScore(matches / Math.max(1, Math.min(leftTokens.size, rightTokens.size)));
}

function tokenSet(value) {
  return new Set(normalizeTask6Text(value)
    .split(' ')
    .filter((token) => token.length > 2 && !task6StopWords.has(token)));
}

function normalizePhraseList(value) {
  return (Array.isArray(value) ? value : [])
    .map((item) => normalizeTask6Text(item))
    .filter(Boolean);
}

function normalizeTask6Text(value) {
  return String(value || '')
    .toLowerCase()
    .replace(/&/g, ' and ')
    .replace(/[^a-z0-9]+/g, ' ')
...
function trimSummaryToWord(value, maxChars) {
...
function looksLikeFirstPersonServiceStory(text) {
  return /\b(I|my|me|we|our)\b/i.test(text)
    && /\b(system|portal|algorithm|score|office|agency|department|worker|caseworker|application|request|benefits|housing|permit|transit|school|service)\b/i.test(text);
}
...
function extractDatePhrases(text) {
  return [
    ...
    ...text.matchAll(/\b(?:last year|weeks|months|one day|next morning|same morning|same week|this week|today|yesterday|tomorrow)\b/gi),
  ].map((match) => match[0]);
}
