...
async function tryRunTask25Locally(text) {
  ...
  try {
    ...
    if (!row) return null;
    return task25LocalPayload(row, {
      text,
      impactFallback: await classifyImpact(text),
    });
  } catch (error) {
...
async function classifyImpact(text) {
  ...
  try {
    const output = await zeroShot(TASK2_MODEL, process.env.ML_IMPACT_ENDPOINT || process.env.ML_BART_ENDPOINT || process.env.ML_DEBERTA_ENDPOINT, text, Object.values(impactLabels), 'This public service story has this impact: {}', true);
