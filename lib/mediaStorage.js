...
function dataUrlResponse({ request, dataUrl, cacheControl = 'private, max-age=0, must-revalidate', filePrefix = 'media' }) {
  const match = /^data:([^;,]+);base64,(.+)$/s.exec(dataUrl || '');
  if (!match) return null;

  const [, contentType, payload] = match;
  const buffer = Buffer.from(payload, 'base64');
  const fileName = `${filePrefix}.${extensionForContentType(contentType)}`;
  const baseHeaders = {
