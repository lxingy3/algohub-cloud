...
export function mediaUploadUserPrefix(userId) {
  const safeUserId = String(userId || '').replace(/[^a-zA-Z0-9_-]/g, '');
  if (!safeUserId) throw new Error('UPLOAD_USER_ID_INVALID');
  return `users/${safeUserId}`;
}

export function isOwnedTestimonyMediaObjectKey({ objectKey, userId }) {
  if (!objectKey || !userId) return false;
  const userPrefix = mediaUploadUserPrefix(userId);
  return ['audio', 'video'].some((kind) => (
    objectKey.startsWith(`testimonies/${kind}/${userPrefix}/`)
  ));
}
...
export function signedMediaUploadPolicyOptions({ contentType, maxBytes, now = Date.now() }) {
  if (!Number.isSafeInteger(maxBytes) || maxBytes <= 0) {
    throw new Error('MEDIA_UPLOAD_MAX_BYTES_INVALID');
  }
...
export async function createSignedMediaUpload({ objectKey, contentType, maxBytes }) {
  const bucket = getFirebaseStorage();
  const [policy] = await bucket.file(objectKey).generateSignedPostPolicyV4(
    signedMediaUploadPolicyOptions({ contentType, maxBytes }),
  );
  return {
