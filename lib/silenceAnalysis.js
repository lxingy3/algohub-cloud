...
export function buildSilenceAnalysis({ algorithms, stories, algorithmEmbeddings = new Map(), storyEmbeddings = new Map() }) {
  ...
  for (const algorithm of algorithms) {
    algorithmsByDomain.set(algorithm.useCase, (algorithmsByDomain.get(algorithm.useCase) || 0) + 1);
  }
  ...
  const rows = algorithms.map((algorithm) => {
    ...
    const domainAlgoCount = algorithmsByDomain.get(algorithm.useCase) || 1;
    ...
    return {
      ...
      possibleReasons: possibleReasons({ approvedCount, expectedVolume, domainGap, semanticGap }),
    };
...
function expectedTestimonyVolume(impactLevel, yearsDeployed, useCase) {
  const base = { HIGH: 20, MEDIUM: 10, LOW: 5 }[impactLevel] || 5;
  ...
  return Math.max(1, Math.trunc(base * Math.max(yearsDeployed, 1) * domainMultiplier));
}
...
function possibleReasons({ approvedCount, expectedVolume, domainGap, semanticGap }) {
  const reasons = [];
  if (!approvedCount) reasons.push('no approved stories linked yet');
  if (approvedCount > 0 && approvedCount < Math.max(3, Math.round(expectedVolume * 0.25))) reasons.push('thin story volume for this system');
  if (domainGap >= 0.6) reasons.push('domain underrepresented in the current story set');
  if (semanticGap >= 0.45) reasons.push('limited semantic coverage of the system scope');
  return reasons;
}
