...
const SUPPORTED_AUDIO_EXTENSIONS = new Set(['.wav', '.mp3', '.webm', '.flac', '.ogg', '.m4a']);
...
function validateSupportedAudioFile(file) {
  ...
  if (SUPPORTED_AUDIO_EXTENSIONS.has(extension) || SUPPORTED_AUDIO_MIME_TYPES.has(type)) return;
  throw new Error('Task 1 supports WAV, MP3, WebM, FLAC, OGG, and M4A audio files.');
}
