...
const inFlightCorpusQueries = new Map();
...
export function normalizeThemes(value) {
  ...
  for (const item of rows) {
    const confidence = typeof item === 'string' || !Number.isFinite(Number(item?.confidence))
      ? null
      : Number(item.confidence);
    const theme = typeof item === 'string'
      ? { theme: item, confidence: null, suggested: true }
      : {
          theme: item?.theme || item?.label || item?.name || '',
          confidence,
          suggested: confidence === null || confidence < 0.75,
        };
    if (!theme.theme || (confidence !== null && confidence <= 0.5)) continue;
    const key = normalizeFacetValue(theme.theme);
    const current = themes.get(key);
    if (!current || (theme.confidence ?? -1) > (current.confidence ?? -1)) themes.set(key, theme);
  }
  return [...themes.values()];
}

export function storyHasTheme(row, theme) {
  const target = normalizeFacetValue(theme);
  if (!target) return true;
  return normalizeThemes(row?.aiThemes).some((item) => normalizeFacetValue(item.theme) === target);
}
...
function findTestimoniesOnce(query) {
  const key = JSON.stringify(query);
  if (inFlightCorpusQueries.has(key)) return inFlightCorpusQueries.get(key);
  const request = prisma.testimony.findMany(query).finally(() => {
    if (inFlightCorpusQueries.get(key) === request) inFlightCorpusQueries.delete(key);
  });
  inFlightCorpusQueries.set(key, request);
  return request;
}
...
export async function getApprovedBriefingCorpus({
  ...
  submissionMethod = '',
  theme = '',
} = {}, { includeExperiences = false, includeExcerpts = false } = {}) {
  ...
  const rows = await findTestimoniesOnce({
    where: {
      jurisdictionId,
    ...
    select: {
      ...
      title: true,
      summary: true,
      ...(includeExcerpts ? { narrativeText: true, transcriptionText: true } : {}),
      ...
      aiThemes: true,
      ...(includeExperiences || includeExcerpts ? { aiExtractedExperiences: true } : {}),
      ...
      neighbourhood: true,
      submittedAt: true,
      brief: { select: { summary: true, reviewStatus: true, generatedAt: true } },
  ...
  return theme ? rows.filter((row) => storyHasTheme(row, theme)) : rows;
}

export function anonymizedExcerpt(row) {
  const text = row.transcriptionText || row.narrativeText || row.brief?.summary || row.summary || '';
...
export async function getAlgorithmLandscape({ agency = '', algorithm = '', domain = '', impactLevel = '', status = [] } = {}) {
  ...
  const algorithms = await prisma.algorithm.findMany({
    where: {
      ...
      ...(algorithm ? { slug: algorithm } : {}),
      ...(agency ? { agencyName: { contains: agency, mode: 'insensitive' } } : {}),
      ...(domain ? { useCase: { contains: domain, mode: 'insensitive' } } : {}),
      ...(impactLevel ? { impactLevel } : {}),
      ...(status?.length ? { status: { in: status } } : {}),
    },
    orderBy: { name: 'asc' },
    select: {
      ...
      yearDeployed: true,
      currentVersion: true,
      testimonyLinks: { where: { testimony: { moderationStatus: APPROVED, publicPosting: true } }, select: { testimonyId: true } },
    },
  });
