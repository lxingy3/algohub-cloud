  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-amber-50 to-slate-100 text-gray-900">
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-6 py-6">
        <form className="-mt-14 space-y-6 rounded-2xl border border-gray-200/80 bg-white/95 p-6 shadow-xl backdrop-blur-sm">
          <div className="relative">
            ...
            <input
              ...
              className="w-full rounded-md border border-gray-200 bg-white py-2 pl-10 pr-3 focus:border-gray-300 focus:outline-none focus:ring-1 focus:ring-gray-300/70"
            />
          </div>
          ...
          <div className="space-y-4 border-t border-gray-200 pt-6">
            ...
            <div className="flex flex-col gap-4 sm:gap-6">
              <label className="flex flex-col gap-2 text-sm font-medium text-gray-600 sm:flex-row sm:items-center">
                ...
                <select name="location" defaultValue={location} className="w-full rounded-md border border-gray-200 bg-white px-3 py-2 sm:w-[220px]">
              ...
              <button className="w-fit rounded-full bg-yellow-500 px-4 py-2 text-sm font-medium text-gray-900 shadow-md">
                Apply filters
              </button>
            </div>
          </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {algorithms.map((algorithm) => (
            <Link key={algorithm.id} href={`/algorithms/${algorithm.slug}`} className="group flex h-full flex-col rounded-lg border border-gray-200 border-l-4 border-l-yellow-500 bg-white p-6 shadow-sm transition-all hover:shadow-lg">
              ...
              <div className="mt-auto flex items-center justify-between pt-5 text-sm text-gray-600">
                <span className="flex items-center gap-1.5">
                  <MapPin className="h-4 w-4 text-gray-400" />
                  {algorithm.location || 'Location not listed'}
                </span>
...
function FilterPill({ href, active, children }) {
  return (
    <Link
      href={href}
      className={active
        ? 'rounded-full bg-yellow-500 px-4 py-2 text-sm font-medium text-gray-900 shadow-md'
        : 'rounded-full border border-gray-200 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50'}
    >
      {children}
    </Link>
  );
