...
export function AddAlgorithmPanel({ currentRole = 'ADMIN' }) {
  ...
  return (
    <>
      ...
      {open ? (
        <div className="fixed inset-0 z-[90] flex items-start justify-center overflow-y-auto bg-slate-950/60 px-4 py-8" role="dialog" aria-modal="true">
          <button type="button" className="fixed inset-0 cursor-default" aria-label="Close add algorithm" onClick={() => setOpen(false)} />
          <div className="relative w-full max-w-4xl rounded-lg bg-white p-5 shadow-2xl">
            <div className="flex items-start justify-between gap-3">
              ...
              <button type="button" onClick={() => setOpen(false)} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100" aria-label="Close add algorithm">
                <X className="h-4 w-4" />
              </button>
            </div>
            <AddAlgorithmForm currentRole={currentRole} />
          </div>
        </div>
      ) : null}
    </>
        ...
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            {algorithm._count?.testimonyLinks < 3 ? (
              <a href="/admin/silence" className="inline-flex min-h-10 items-center rounded-full bg-red-50 px-2 py-1 text-xs font-semibold text-red-800">
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/50 px-4" role="dialog" aria-modal="true">
      <div className="w-full max-w-sm rounded-lg border bg-white p-5 shadow-2xl">
        <div className="flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={onCancel} className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-md text-slate-500 hover:bg-slate-100" aria-label="Close confirmation">
            <X className="h-4 w-4" />
          </button>
        </div>
